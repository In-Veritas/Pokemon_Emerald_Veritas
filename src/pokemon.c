...
void CreateBoxMon(struct BoxPokemon *boxMon, u16 species, u8 level, u8 fixedIV, u8 hasFixedPersonality, u32 fixedPersonality, u8 otIdType, u32 fixedOtId)
{
    ...
    switch (otIdType)
    {
        ...
        default:
        {
            ...
            {
                // Veritas shiny system: 1/8192 base odds, +1 roll per Shiny Charm
                u32 shinyValue;
                u32 maxRolls = BASE_SHINY_ROLLS;
                u32 rolls = 0;
                ...
                if (CheckBagHasItem(ITEM_SHINY_CHARM, 1))
                {
                    ...
                    // Count total Shiny Charms
                    while (CheckBagHasItem(ITEM_SHINY_CHARM, itemCount + 1))
                        itemCount++;

                    // +1 roll per Shiny Charm
                    maxRolls += itemCount;
                }
#endif
...
s32 CalculateBaseDamage(struct BattlePokemon *attacker, struct BattlePokemon *defender, u32 move, u16 sideStatus, u16 powerOverride, u8 typeOverride, u8 battlerIdAtk, u8 battlerIdDef)
{
    ...
    if (defenderHoldEffect == HOLD_EFFECT_DEEP_SEA_SCALE && defender->species == SPECIES_CLAMPERL)
        spDefense *= 2;
    if (attackerHoldEffect == HOLD_EFFECT_LIGHT_BALL && (attacker->species == SPECIES_PIKACHU || attacker->species == SPECIES_PICHU || attacker->species == SPECIES_PLUSLE || attacker->species == SPECIES_MINUN))
        spAttack *= 2;
...
u16 GetBattleBGM(void)
{
    if (gBattleTypeFlags & BATTLE_TYPE_KYOGRE_GROUDON)
        ...
        return MUS_VS_REGI;
    else if (gBattleTypeFlags & (BATTLE_TYPE_LINK | BATTLE_TYPE_RECORDED_LINK | BATTLE_TYPE_SECRET_BASE))
    {
    ...
    else if (gBattleTypeFlags & BATTLE_TYPE_TRAINER)
    {
        ...
        if (gTrainerBattleOpponent_A == TRAINER_EXCLSIOR || gTrainerBattleOpponent_A == TRAINER_CHAMPION_EXCLSIOR)
...
void SetWildMonHeldItem(void)
{
    if (!(gBattleTypeFlags & (BATTLE_TYPE_LEGENDARY | BATTLE_TYPE_TRAINER | BATTLE_TYPE_PYRAMID | BATTLE_TYPE_PIKE)))
    {
        ...
        // Pearl-holding species: 35% chance for Pearl as common item
            ...
            && GetMonAbility(&gPlayerParty[0]) == ABILITY_COMPOUND_EYES)
        {
            ...
            // Pearl-holding species with Compound Eyes
            if (species == SPECIES_CLAMPERL || species == SPECIES_SPOINK || species == SPECIES_STARYU)
            {
                chanceNoItem = 30; // 30% no item, 45% Pearl, 25% rare
                chanceNotRare = 75;
            }
        }
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    // Clamperl rare slot: 50/50 DeepSeaScale or DeepSeaTooth
                    if (species == SPECIES_CLAMPERL)
                    {
                        evoItem = (Random() % 2) ? ITEM_DEEP_SEA_SCALE : ITEM_DEEP_SEA_TOOTH;
                        SetMonData(&gEnemyParty[0], MON_DATA_HELD_ITEM, &evoItem);
                    }
                    else
                    {
                        SetMonData(&gEnemyParty[0], MON_DATA_HELD_ITEM, &gSpeciesInfo[species].itemRare);
