...
enum
{
    ...
    MENUITEM_WORLD_SHORTPOKECENTER,
    MENUITEM_WORLD_DEVONSCOPE,
...
static void DrawChoices_ShortPokecenter(int selection, int y);
static void DrawChoices_DevonScope(int selection, int y);
...
} static const sItemFunctionsMain[MENUITEM_MAIN_COUNT] =
{
    ...
    [MENUITEM_MAIN_SEVOLUME]     = {DrawChoices_SEVolume,    ProcessInput_Options_Five},
...
} static const sItemFunctionsWorld[MENUITEM_WORLD_COUNT] =
{
    ...
    [MENUITEM_WORLD_SHORTPOKECENTER]    = {DrawChoices_ShortPokecenter, ProcessInput_Options_Two},
    [MENUITEM_WORLD_DEVONSCOPE]         = {DrawChoices_DevonScope,      ProcessInput_Options_Two},
...
// SE Volume: maps display index (Quiet=0,Low=1,Medium=2,High=3,Loud=4) <-> VAR_SE_VOLUME value
static const u8 sSEVolumeDisplayToVar[] = {1, 3, 0, 4, 2};
static const u8 sSEVolumeVarToDisplay[] = {2, 0, 4, 1, 3};
...
static const u8 sText_ShortPokecenter[]     = _("SHORT POKéCENTER");
static const u8 sText_DevonScope[]          = _("DEVON SCOPE");
static const u8 *const sOptionMenuItemsNamesWorld[MENUITEM_WORLD_COUNT] =
{
    ...
    [MENUITEM_WORLD_SHORTPOKECENTER]    = sText_ShortPokecenter,
    [MENUITEM_WORLD_DEVONSCOPE]         = sText_DevonScope,
...
#endif
        case MENUITEM_WORLD_SHORTPOKECENTER: return TRUE;
        case MENUITEM_WORLD_DEVONSCOPE:      return FlagGet(FLAG_RECEIVED_DEVON_SCOPE);
...
static const u8 sText_Desc_SEVolume_Low[]        = _("Play the select ding at low\nvolume.");
static const u8 sText_Desc_SEVolume_Medium[]     = _("Play the select ding at half\nvolume. Recommended setting.");
static const u8 sText_Desc_SEVolume_High[]       = _("Play the select ding at high\nvolume.");
static const u8 sText_Desc_SEVolume_Loud[]       = _("Play the select ding at full\nvolume. Original experience.");
static const u8 *const sOptionMenuItemDescriptionsMain[MENUITEM_MAIN_COUNT][5] =
{
    [MENUITEM_MAIN_TEXTSPEED]       = {sText_Desc_TextSpeed,            sText_Empty,                    sText_Empty,                sText_Empty,                sText_Empty},
    [MENUITEM_MAIN_SOUND]           = {sText_Desc_SoundMono,            sText_Desc_SoundStereo,         sText_Empty,                sText_Empty,                sText_Empty},
    [MENUITEM_MAIN_FRAMETYPE]       = {sText_Desc_FrameType,            sText_Empty,                    sText_Empty,                sText_Empty,                sText_Empty},
    [MENUITEM_CUSTOM_FONT]          = {sText_Desc_FontType_Hoenn,       sText_Desc_FontType_Kanto,      sText_Empty,                sText_Empty,                sText_Empty},
    [MENUITEM_MAIN_NICKNAME]        = {sText_Desc_Nickname_Hide,        sText_Desc_Nickname_Show,       sText_Empty,                sText_Empty,                sText_Empty},
    [MENUITEM_MAIN_SEVOLUME]        = {sText_Desc_SEVolume_Quiet,       sText_Desc_SEVolume_Low,        sText_Desc_SEVolume_Medium, sText_Desc_SEVolume_High,   sText_Desc_SEVolume_Loud},
    [MENUITEM_MAIN_CANCEL]          = {sText_Desc_Save,                 sText_Empty,                    sText_Empty,                sText_Empty,                sText_Empty},
    [MENUITEM_MAIN_BUTTONMODE]      = {sText_Desc_ButtonMode,           sText_Desc_ButtonMode_LA,       sText_Empty,                sText_Empty,                sText_Empty},
};
...
static const u8 sText_Desc_ShortPokecenter_On[]         = _("Skip the yes/no question at the\nPOKéMON CENTER.");
static const u8 sText_Desc_ShortPokecenter_Off[]        = _("Original experience.\nAlways ask the yes/no question.");
static const u8 sText_Desc_DevonScope_On[]              = _("Show FEEBAS tiles on ROUTE 119.\nLower encounter rate (5%).");
static const u8 sText_Desc_DevonScope_Off[]             = _("Hide FEEBAS tiles. Vanilla mechanics.\nHigher encounter rate (50%).");
static const u8 *const sOptionMenuItemDescriptionsWorld[MENUITEM_WORLD_COUNT][4] =
{
    ...
    [MENUITEM_WORLD_SHORTPOKECENTER]    = {sText_Desc_ShortPokecenter_On,   sText_Desc_ShortPokecenter_Off},
    [MENUITEM_WORLD_DEVONSCOPE]         = {sText_Desc_DevonScope_On,        sText_Desc_DevonScope_Off},
...
static const u8 *const sOptionMenuItemDescriptionsDisabledWorld[MENUITEM_WORLD_COUNT] =
{
    ...
    [MENUITEM_WORLD_SHORTPOKECENTER]    = sText_Empty,
    [MENUITEM_WORLD_DEVONSCOPE]         = sText_Empty,
...
void CB2_InitOptionPlusMenu(void)
{
    ...
    case 6:
        ...
        sOptions->sel[MENUITEM_MAIN_SEVOLUME]    = (VarGet(VAR_SE_VOLUME) < 5) ? sSEVolumeVarToDisplay[VarGet(VAR_SE_VOLUME)] : 2;
        ...
        sOptions->sel_world[MENUITEM_WORLD_SHORTPOKECENTER]     = !FlagGet(FLAG_ENABLE_SHORT_POKECENTER);   // Used the inverse to align with ON/OFF Buttons
        sOptions->sel_world[MENUITEM_WORLD_DEVONSCOPE]          = !FlagGet(FLAG_RECEIVED_DEVON_SCOPE);      // ON = tiles visible, OFF = vanilla
...
static void Task_OptionMenuSave(u8 taskId)
{
    ...
    VarSet(VAR_SE_VOLUME, sSEVolumeDisplayToVar[sOptions->sel[MENUITEM_MAIN_SEVOLUME]]);
    ...
    sOptions->sel_world[MENUITEM_WORLD_SHORTPOKECENTER]     == 0 ? FlagSet(FLAG_ENABLE_SHORT_POKECENTER) : FlagClear(FLAG_ENABLE_SHORT_POKECENTER); // Used the inverse to align with other similar options.
    // Devon Scope toggle - also remix Feebas tiles when changed
    if (sOptions->sel_world[MENUITEM_WORLD_DEVONSCOPE] == 0)
        FlagSet(FLAG_RECEIVED_DEVON_SCOPE);
    else
        FlagClear(FLAG_RECEIVED_DEVON_SCOPE);
    GetFeebasTiles(); // Remix Feebas tile locations when Devon Scope is toggled
...
static const u8 sText_SEVolume_Low[]    = _("LOW");
static const u8 sText_SEVolume_Medium[] = _("MEDIUM");
static const u8 sText_SEVolume_High[]   = _("HIGH");
static const u8 sText_SEVolume_Loud[]   = _("LOUD");
static const u8 *const sSEVolumeStrings[] = {sText_SEVolume_Quiet, sText_SEVolume_Low, sText_SEVolume_Medium, sText_SEVolume_High, sText_SEVolume_Loud};
static void DrawChoices_SEVolume(int selection, int y)
{
    bool8 active = CheckConditions(MENUITEM_MAIN_SEVOLUME);
    DrawOptionMenuChoiceStrings(selection, y, active, sSEVolumeStrings, 5);
}
...
static const u8 sText_ShortPokecenter_On[]   = _("ON");
static const u8 sText_ShortPokecenter_Off[]   = _("OFF");
static const u8 *const sShortPokecenterStrings[] = {sText_ShortPokecenter_On, sText_ShortPokecenter_Off};
static void DrawChoices_ShortPokecenter(int selection, int y)
{
    bool8 active = CheckConditions(MENUITEM_WORLD_SHORTPOKECENTER);
    DrawOptionMenuChoiceStrings(selection, y, active, sShortPokecenterStrings, 2);
}

static const u8 sText_DevonScope_On[]   = _("ON");
static const u8 sText_DevonScope_Off[]  = _("OFF");
static const u8 *const sDevonScopeStrings[] = {sText_DevonScope_On, sText_DevonScope_Off};
static void DrawChoices_DevonScope(int selection, int y)
{
    bool8 active = CheckConditions(MENUITEM_WORLD_DEVONSCOPE);
    DrawOptionMenuChoiceStrings(selection, y, active, sDevonScopeStrings, 2);
}
