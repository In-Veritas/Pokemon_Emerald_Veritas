...
static bool8 CheckFeebas(void)
{
    ...
    if (gSaveBlock1Ptr->location.mapGroup == MAP_GROUP(ROUTE119) && gSaveBlock1Ptr->location.mapNum == MAP_NUM(ROUTE119))
    {
        ...
        random = Random() % 100;
        // Feebas encounter rate depends on Devon Scope toggle:
        // - Devon Scope OFF: 75% chance (Veritas boosted)
        // - Devon Scope ON (tiles visible): 5% chance (balanced since tiles are visible)
        if (random > 74 && !FlagGet(FLAG_RECEIVED_DEVON_SCOPE)) // 75% Feebas chance
            return FALSE;
        else if (random > 4 && FlagGet(FLAG_RECEIVED_DEVON_SCOPE)) // 5% after Devon Scope
            return FALSE;
...
u8 PickWildMonNature(void)
{
    ...
    // check synchronize for a Pokémon with the same ability (100% chance)
    if (!GetMonData(&gPlayerParty[0], MON_DATA_SANITY_IS_EGG)
