...
// Migrates old saves that used playerLookStyle in SaveBlock2 to use FLAG_PLAYER_STYLE_RS instead
static void MigratePlayerLookStyleToFlag(void)
{
    // Check if old playerLookStyle field has a value (RS style = 1)
    // and migrate it to the flag system
    if (gSaveBlock2Ptr->playerLookStyle != 0)
    {
        FlagSet(FLAG_PLAYER_STYLE_RS);
        // Clear the old field so migration doesn't run again
